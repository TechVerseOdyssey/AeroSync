...
### 2.1 Workspace 成员（7 个 crate）

```text
AeroSync workspace (root Cargo.toml [workspace].members)
...
└── aerosync-rendezvous         # 独立 crate（lib+bin）：RFC-004 控制面（SQLite 注册 + JWT，HTTP /v1/peers/*）
```
...
| 2.3  | `HistoryStore impl HistoryStorage`（后续在 Phase 3.4b 迁到 `aerosync-infra/src/history.rs`） | `158dc07` |
...
### 2.3 RFC-004 控制面 week 1（`main` 当前实现）
...
**v0.3.0 已落地**：新增 `session` 模块（`TransferSession` 聚合根 + `SessionId`、`SessionKind`、`SessionStatus`）、`manifest` 模块（`FileManifest`、`FileEntry`、`Hash`、`ChunkPlan`）、`receipt` 模块（从根 crate 迁入纯状态机部分）。
...
**v0.3.0-rc1 收口状态**：`history` 模块已迁到 `aerosync-infra/src/history.rs`，并以 `spawn_watch_bridge` 自由函数消除了对根 crate `Receipt` 的物理耦合；`config` 的 TOML 加载逻辑仍保留在 CLI / Python 各自边界，不单独抽 crate。
...
| `wan::rendezvous` / `wan::relay` | `rendezvous`：RFC-004 week-1 客户端 + 解析（`main`，feature 开则非 no-op）；`relay` 仍为占位 | features `wan-rendezvous` / `wan-relay` |
...
- **Application 层（root crate）通过 trait 注入消费 infra**——v0.3.0-rc1 已完成 `TransferEngine` / `AutoAdapter` / `HttpTransfer` 对 `Arc<dyn ResumeStorage>` / `Arc<dyn HistoryStorage>` 的接入，并保留具体类型 builder 作为向后兼容入口。
...
| `HistoryStore impl HistoryStorage` | `aerosync-infra/src/history.rs`（由 commit `158dc07` 铺垫，Phase 3.4b 完成迁移） | ✅ trait 已 impl，物理迁移已完成 |
| `TransferEngine` / `AutoAdapter` / `HttpTransfer` 通过 `Arc<dyn …>` 注入 | `src/core/transfer.rs`、`src/protocols/adapter.rs`、`src/protocols/http.rs` | ✅ 已完成；保留具体类型 builder 作为兼容层 |
...
| 工作项 | 目标 crate / 文件 | 状态 |
|--------|------------------|------|
| TransferEngine 接受 `TransferSession`（每 `send` 建会话 + 元数据 `session_id`） | `aerosync/src/core/transfer.rs` | **已落地**（3.4e） |
| 接收端 `session_id`（wire；Python `Receipt`、Rust `IncomingFile::session_id`）与 `FileReceiver::received_ids_for_session` 进程内索引 | `aerosync-py` / `IncomingFile` / `server.rs` | **已落地**（3.5 实用集；索引为内存、按进程生命周期） |
| PyO3 `Receipt.session_id` | `aerosync-py/src/receipt.rs` | **已落地**（3.4f） |
| Wire proto `Metadata.session_id` | `aerosync-proto/proto/aerosync/wire/v1.proto` 字段 10 | **已落地**（3.4g） |
| R2 直连（裸 `peer@`：initiate + 信令 + `punch_at` + 共享 UDP/QUIC，`AutoAdapter::try_r2_upload`；`aerosync-rendezvous` 信令与客户端 `exchange_*` 等） | `aerosync` / `aerosync-rendezvous` 等 | **已落地**；详见 [RFC-004](rfcs/RFC-004-wan-rendezvous.md) *Implementation status* |
| RFC-004 其余（R3 中继数据面、MCP/CLI 全量 WAN 扩展、更完备 ACL 等） | 全 workspace | **未结**，见 [RFC-004 任务表](rfcs/RFC-004-wan-rendezvous.md) |

### 12.2 v0.5+：可选基础设施扩展

| 工作项 | 目标 crate / 文件 | 备注 |
|--------|------------------|------|
| SQLite 后端 `HistoryStorage` 实现 | `aerosync-infra/src/history_sqlite.rs`（`SqliteHistoryStore`）| **已落地**；引擎仍默认 JSONL，换后端需显式构造并注入 `Arc<dyn HistoryStorage>`（配置项后续） |
