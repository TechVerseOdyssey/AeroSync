...
│ ProgressMonitor│       │                         │
└────────────────┘       └─────────────────────────┘
       (both live in the `aerosync` library crate)
```
...
```
AeroSync/
├── Cargo.toml              # workspace 根（aerosync 主 crate + aerosync-mcp）
├── src/                    # aerosync crate（lib + CLI bin）
│   ├── lib.rs              # 库根，re-export 常用类型；声明 core + protocols 子模块
│   ├── main.rs             # CLI 入口（aerosync 二进制）
...
└── aerosync-mcp/           # 独立 crate：MCP server，依赖 aerosync 库
    └── src/
        ├── lib.rs
