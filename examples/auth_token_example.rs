...
fn example_basic_token() -> Result<()> {
    ...
    // 创建 Token 管理器
    let token_manager = TokenManager::new("my-secret-key-12345".to_string())?
        .with_lifetime(24); // 24 小时有效期
...
fn example_auth_manager() -> Result<()> {
    ...
    // 测试无效 Token
    let is_authenticated_invalid = auth_manager.authenticate(
        Some("invalid-token"),
        client_ip
    )?;
...
fn example_http_middleware() -> Result<()> {
    ...
    // 模拟 HTTP 请求验证
    println!("模拟 HTTP 请求...");
    let is_authenticated = middleware.authenticate_http_request(
        Some(&auth_header),
        "192.168.1.50"
    )?;
    ...
    // 模拟无 Token 的请求
    println!("\n模拟无 Token 的 HTTP 请求...");
    let is_authenticated_no_token = middleware.authenticate_http_request(
        None,
        "192.168.1.50"
    )?;
...
fn example_file_upload_service() -> Result<()> {
    ...
    let can_upload = middleware.authenticate_http_request(
        Some(&auth_header),
        "203.0.113.42"
    )?;
    ...
    // 4. 未授权用户尝试访问
    println!("\n4. 未授权用户尝试上传文件...");
    let unauthorized_upload = middleware.authenticate_http_request(
        None,
        "198.51.100.1"
    )?;
